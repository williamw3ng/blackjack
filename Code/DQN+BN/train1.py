...
def play(player, game, buffer, epsilon, config):
    ...
    while not terminal:
        ...
        good_cards = game.get_good_cards()
        good_cards_e = np.expand_dims(np.expand_dims(good_cards, axis=0), axis=0)
        good_belief = belief * good_cards_e
        # obv_e = np.concatenate([obv_e, good_cards_e], axis=-1)
        q_values = player.primary_model.get_q_values(obv_e, good_belief)
...
def main():
    ...
    # onfig.num_memory = int(input("memory dimension: "))
    ckpt_path = "CKPT1"
    ...
    player.restore_ckpt(os.path.join(save_path, ckpt_path), sess)
    ...
    for episode in range(int(config.start_iteration) + 1, int(config.end_iteration) + 1):
        ...
        epsilon = 1 if episode < 200 else epsilon
        ...
        if episode % config.save_iteration == 0:
            player.save_ckpt(sess, os.path.join(save_path, ckpt_path), global_step=episode)

    player.save_ckpt(sess, os.path.join(save_path, ckpt_path), global_step=int(config.end_iteration))
